...
export const Route = createFileRoute("/premium")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Unlock the full SmartyMove experience. Daily corrective program, re-tests, Movement Age trajectory, and your full 2-week training schedule — €4.99/month." },
      { property: "og:title", content: "SmartyMove Premium" },
      { property: "og:description", content: "Daily corrective program, re-tests, full schedule — €4.99/month. Cancel anytime." },
    ...
    <div className="flex min-h-[100dvh] w-full flex-col" style={{ background: "#E7ECEC", color: "#14213A" }}>
      ...
      <main className="mx-auto w-full max-w-[760px] px-5 pb-8 pt-5">
        ...
        <section className="mt-4 rounded-3xl bg-white p-6 shadow" style={{ border: "1px solid #E5EAEC" }}>
          <h2 className="text-lg font-extrabold" style={{ color: "#14213A" }}>What's included</h2>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <Feature icon={<Dumbbell className="h-4 w-4" />} title="Full 2-week program">All 7 exercises per session, every day, instead of just a preview.</Feature>
            <Feature icon={<RotateCcw className="h-4 w-4" />} title="Re-tests & rescans">Retake the screen every 14 days to build a fresh program around your new results.</Feature>
            <Feature icon={<LineChart className="h-4 w-4" />} title="Future Projection">See where your Movement Age is heading — and what to do to lower it.</Feature>
            <Feature icon={<Camera className="h-4 w-4" />} title="All add-on tests">Joint-specific assessments for ankle, knee, hip, low back, shoulder, elbow, wrist.</Feature>
            <Feature icon={<Sparkles className="h-4 w-4" />} title="Root-cause clustering">Smart prioritization: fix one root cause that resolves multiple symptoms.</Feature>
            <Feature icon={<Zap className="h-4 w-4" />} title="Progress tracking">Mark workouts complete, keep a streak, share your wins.</Feature>
          </div>
        </section>

        <section className="mt-4 rounded-3xl bg-white p-6 shadow" style={{ border: "1px solid #E5EAEC" }}>
          <h2 className="text-lg font-extrabold" style={{ color: "#14213A" }}>Why it's worth it</h2>
          <ul className="mt-3 space-y-2.5 text-sm" style={{ color: "#3B4A63", lineHeight: 1.55 }}>
            <Li>One physio session typically costs €50–€90. SmartyMove gives you a screen + daily corrective plan for an entire month at €4.99.</Li>
            <Li>Built around <strong>root causes</strong>, not symptoms — your program targets the one limitation that fixes the most things at once.</Li>
            <Li>5 minutes a day. No equipment. No commute. Works around your life.</Li>
            <Li>Re-tests every 14 days keep the program honest — you see real progress in your Movement Score.</Li>
          </ul>
        </section>

        <section className="mt-4 rounded-3xl bg-white p-6 shadow" style={{ border: "1px solid #E5EAEC" }}>
          <h2 className="text-lg font-extrabold" style={{ color: "#14213A" }}>Free vs Premium</h2>
          <div className="mt-3 overflow-hidden rounded-2xl" style={{ border: "1px solid #E5EAEC" }}>
            <Row head label="" left="Free" right="Premium" />
            <Row label="Movement Screen" left={<Check className="mx-auto h-4 w-4" />} right={<Check className="mx-auto h-4 w-4" />} />
            <Row label="Score & Movement Age" left={<Check className="mx-auto h-4 w-4" />} right={<Check className="mx-auto h-4 w-4" />} />
            <Row label="Preview the program" left={<Check className="mx-auto h-4 w-4" />} right={<Check className="mx-auto h-4 w-4" />} />
            <Row label="Open daily workouts" left="—" right={<Check className="mx-auto h-4 w-4" />} />
            <Row label="Mark sessions complete" left="—" right={<Check className="mx-auto h-4 w-4" />} />
            <Row label="Re-tests & rescans" left="—" right={<Check className="mx-auto h-4 w-4" />} />
            <Row label="Future Projection" left="—" right={<Check className="mx-auto h-4 w-4" />} />
            <Row label="All add-on joint tests" left="—" right={<Check className="mx-auto h-4 w-4" />} />
          </div>
        </section>
  ...
  return (
    <div style={{ background: "#F1F5F4", border: "1px solid #D9E0E2", borderRadius: 14, padding: "12px 14px" }}>
      <div className="flex items-center gap-2" style={{ color: "#0E7C86", fontWeight: 700, fontSize: 14 }}>
        <span className="grid place-items-center" style={{ width: 24, height: 24, borderRadius: 8, background: "#0E7C86", color: "#fff" }}>{icon}</span>
        {title}
      </div>
...
function Row({ label, left, right, head }: { label: string; left: React.ReactNode; right: React.ReactNode; head?: boolean }) {
  const base = { background: head ? "#0E7C86" : "#fff", color: head ? "#fff" : "#14213A", fontWeight: head ? 700 : 500 } as const;
  return (
    <div className="grid grid-cols-[1.5fr_1fr_1fr] items-center" style={{ borderTop: head ? "none" : "1px solid #E5EAEC", ...base }}>
      <div className="px-3 py-2.5 text-sm">{label}</div>
      <div className="px-3 py-2.5 text-center text-sm" style={{ borderLeft: "1px solid #E5EAEC", color: head ? "#fff" : "#6B7A90" }}>{left}</div>
      <div className="px-3 py-2.5 text-center text-sm font-semibold" style={{ borderLeft: "1px solid #E5EAEC", color: head ? "#fff" : "#0E7C86" }}>{right}</div>
    </div>
  );
}