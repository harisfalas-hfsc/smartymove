...
export function evaluateRescan(user: User, status: ProgramStatus | null): RescanSuggestion | null {
  ...
  if (user.goal && latest.goalAtScan && user.goal !== latest.goalAtScan) {
    return {
      ...
      message: "Your new goal changes what we're looking for in your movement. A fresh scan gives your program a clean starting point.",
      cta: "Re-scan now",
    };
  }
  ...
  if (fb?.changed && daysBetween(fb.date, now) <= 3) {
    return {
      ...
      message: "You mentioned something felt different after your last session — a quick re-scan will show whether your scores have moved.",
      cta: "Re-scan now",
    };
  }
  ...
  if (phase.phase === "restore" && foundationWeeksDone && foundationSessionsDone) {
    ...
    return {
      ...
      message: `Great work. Let's re-scan to see if your ${areaLabel} has improved and whether you're ready to move to the Build stage.`,
  ...
  if (sessions.length >= 3) {
    ...
    if (Math.abs(delta1) <= 2 && Math.abs(delta2) <= 2 && daysSince >= 14) {
      return {
        ...
        message: "That's okay — some restrictions take longer. Consider a conversation with a movement specialist, or we can adjust your program focus and try again.",
  ...
  if (daysSince >= 14) {
    return {
      ...
      message: "Your 2-week program is complete. Rescan now to measure your progress and unlock your next program.",
      cta: "Start Movement Screen",
    };
  }
